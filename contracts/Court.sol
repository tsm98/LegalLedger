...
contract Court {
    ...
    function registerLawyer(string memory _name, string memory _bar_id, uint _phone, string memory _email, address _addr, string memory _pubkey) public {
        Lawyer memory l = Lawyer(_name, _bar_id, _phone, _email, _addr, _pubkey);
        lawyers.push(l);
        emit lawyerRegistered(lawyers.length-1);
    }
    function registerJudge(string memory _name, string memory _bar_id, uint _phone, string memory _email, address _addr, string memory _pubkey) public {
        Judge memory j = Judge(_name, _bar_id, _phone, _email, _addr, _pubkey);
        judges.push(j);
